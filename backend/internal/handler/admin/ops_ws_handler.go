...
var upgrader = websocket.Upgrader{
	...
	// - The frontend passes ["apipool-admin", "jwt.<token>"].
	// - We always select "apipool-admin" so the token is never echoed back in the handshake response.
	Subprotocols: []string{"apipool-admin"},
}
